...
class ProtectFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    for entry in self._async_current_entries():
        ...
        if await self._handle_existing_entry(
            entry, mac, direct_connect_domain, source_ip
        ):
            return self.async_abort(reason="already_configured")

    return await self.async_step_discovery_confirm()

async def _handle_existing_entry(
    self, entry, mac, direct_connect_domain, source_ip
):
