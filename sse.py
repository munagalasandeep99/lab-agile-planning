...
def _update_cost(self) -> None:
    ...
    if state_class not in SUPPORTED_STATE_CLASSES:
        self._handle_unsupported_state_class(state_class, energy_state.entity_id)
        return

    energy, energy_unit = self._get_energy_and_unit(energy_state)
    if energy is None or energy_unit is None:
        return

    energy_price, energy_price_unit = self._get_energy_price_and_unit()

    if energy_price is None or energy_price_unit is None:
        return
    ...
    if self._energy_meter_reset(energy_state):
        self._reset(energy_state)
        return

    self._calculate_cost(energy_state, energy, energy_unit, energy_price, energy_price_unit)

@staticmethod
def _handle_unsupported_state_class(state_class, entity_id):
    """Handle unsupported state class."""
    if not self._wrong_state_class_reported:
        self._wrong_state_class_reported = True
        _LOGGER.warning("Found unexpected state_class %s for %s", state_class, entity_id)

def _get_energy_and_unit(self, energy_state):
    """Get energy value and unit from the energy state."""
    energy_unit = energy_state.attributes.get(ATTR_UNIT_OF_MEASUREMENT)
    if energy_unit is None or energy_unit not in VALID_UNITS:
        self._handle_invalid_unit(energy_unit, energy_state.entity_id)
        return None, None
...
def _handle_invalid_unit(self, energy_unit, entity_id):
    """Handle invalid energy unit."""
    if not self._wrong_unit_reported:
        self._wrong_unit_reported = True
        _LOGGER.warning("Found unexpected unit %s for %s", energy_unit, entity_id)

def _get_energy_price_and_unit(self):
    """Get energy price and unit."""
    if self._config["entity_energy_price"] is not None:
        energy_price_state = self.hass.states.get(self._config["entity_energy_price"])
        if energy_price_state:
            energy_price_unit = energy_price_state.attributes.get(ATTR_UNIT_OF_MEASUREMENT, "").partition("/")[2]
            energy_price = self._get_energy_price(energy_price_state, energy_price_unit)
            return energy_price, energy_price_unit

    energy_price_unit = self._get_default_price_unit()
    energy_price = float(self._config["number_energy_price"])
    return energy_price, energy_price_unit

def _get_default_price_unit(self):
    """Get the default energy price unit."""
    if self._adapter.source_type == "grid":
        return UnitOfEnergy.KILO_WATT_HOUR
    elif self._adapter.source_type == "gas":
        return None
    elif self._adapter.source_type == "water":
        return UnitOfVolume.CUBIC_METERS if self.hass.config.units is METRIC_SYSTEM else UnitOfVolume.GALLONS

def _energy_meter_reset(self, energy_state):
    """Check if the energy meter was reset."""
    if energy_state.attributes.get(ATTR_LAST_RESET) != self._last_energy_sensor_state.attributes.get(ATTR_LAST_RESET):
        return True

    return False

def _calculate_cost(self, energy_state, energy, energy_unit, energy_price, energy_price_unit):
    """Calculate incurred costs."""
    old_energy_value = float(self._last_energy_sensor_state.state)
    cur_value = float(self._attr_native_value)

    if energy_price_unit is None:
        converted_energy_price = energy_price
    else:
        converter = self._get_unit_converter(energy_unit)
...
def _get_unit_converter(self, energy_unit):
    """Get the unit converter based on energy unit."""
    return unit_conversion.EnergyConverter.convert if energy_unit in VALID_UNITS else unit_conversion.VolumeConverter.convert
